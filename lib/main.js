...
function evaluateFix(fix, script, csp, contentReplace) {
    switch(fix) {
        ...
        case "gh-script-integrity":
            // thanks https://github.com/martok/palefill/issues/29#issuecomment-1172984535
        ...
        case "gh-script-optchain":
            let replacer = function (mch, p1, p2) {
                ...
                const try_txt = (e?
                    `const _1 = ${p}; const _2 = _1${p1.slice(p.length)}; return (typeof _2 === "function" && "constructor" in _2)?_2.bind(_1):_2`:
                    "return " + p1);
                // limit the catch: thanks https://github.com/martok/palefill/issues/29#issuecomment-1174836915
                const catch_txt = `if (e.name == "ReferenceError" || /\\sis\\s+(null|undefined|not a function)/.test(e.message||"")) {return ${p2 != null? "(()=>undefined)": "undefined"};}; throw e`;
                const ret = `((()=>{try {${try_txt};} catch (e) {${catch_txt};}})())${p2==null?"":p2}`;
                print("modifying " + mch + " to " + ret);
                return ret;
            }

            // return(this.anchorNode ...)?.getAttribute(...)
            contentReplace.push(
                [/return|(\(this\.anchorNode (?:[^()]|\([^()]*\))+\)(?:\??\.(?:[a-zA-Z_$][a-zA-Z_$0-9]*))+)(\()?/g,
                 replacer]);
            contentReplace.push(
                ...
                [/(?:if\(|return\(|g\.split\(E\)\.filter\(|((?:(?:[a-zA-Z_$][a-zA-Z_$0-9]*|\(0,(?:[a-zA-Z_$][a-zA-Z_$0-9]*\??\.)*[a-zA-Z_$][a-zA-Z_$0-9]*\)|\(await\(0,(?:[a-zA-Z_$][a-zA-Z_$0-9]*\??\.)*[a-zA-Z_$][a-zA-Z_$0-9]*\)\([^{)]*\)\))(?:\((?:(?:\$\{|[^{)])*|`[a-zA-Z_$0-9={}\[\]()"'=./+-]*`|\/(?:\\\/|[^\/])+\/[gi]?)\)|\[(?:\$\{|[^{\]])*\])*\??\.(?:\([^)]*\)|\[[^\]]*\])*)+(?:[a-zA-Z_$][a-zA-Z_$0-9]*)?))(\()?/g,
                 replacer]);
            // const o=(0,$t.P)(t.querySelector(".js-code-editor"))?.editor, as.P(...)?.editor
            contentReplace.push(
                [/((?:\([^)]*\)|(?:[a-zA-Z_$][a-zA-Z_$0-9]*\.)*[a-zA-Z_$][a-zA-Z_$0-9]*)\((?:[a-zA-Z_$][a-zA-Z_$0-9]*\.)*[a-zA-Z_$][a-zA-Z_$0-9]*\([^)]*\)\)\?\.[a-zA-Z_$][a-zA-Z_$0-9]*)(\()?/g,
                 replacer]);
...
function supersededFixes(service) {
    // Declare fixes that are not needed any more by certain versions of the ecosystem.
...
class MergedFix {
    ...
    ensureCompiled() {
        ...
        // for each selected fix, append it and its consequences to a list
...
class PolyfillService {
    constructor() {
        ...
        this.hasQueueMicrotask = this._checkSyntax("queueMicrotask.arguments");
    ...
    modifyRequestData(data, activeFixes) {
        ...
        if (activeFixes.scripts) {
            data = data.replace(/<head[^>]*>/, "$&" + activeFixes.scripts);
        }
        return data;
    }
}
...
class HTTPObserver {
    ...
    isHTMLDocument(subject) {
        ...
        return (subject.loadInfo.externalContentPolicyType == nsIContentPolicy.TYPE_DOCUMENT ||
                ...
                subject.loadInfo.externalContentPolicyType == nsIContentPolicy.TYPE_FETCH) &&
